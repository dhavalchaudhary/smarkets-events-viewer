...
import {
  mockEventsData,
  mockPopularEventIds,
  mockEventDataAPIResponse
} from '../../mocks'
import {
  PopularEventsViewer,
  PopularEventsViewerProps
} from './popular-events-viewer'
...
describe('PopularEventsViewer', () => {
  ...
  it('displays the data when present', () => {
    render(
      <MemoryRouter>
        <PopularEventsViewer {...mockBaseProps} />
      </MemoryRouter>
    )

    expect(
      screen.getByText(mockBaseProps.eventsData[mockEventId].name)
    ).toBeVisible()
  })
  it('fetches the data when not present and updates global state', async () => {
    ...
    ;(fetchPopularEventsIds as jest.Mock).mockResolvedValue(
      mockPopularEventIdsApiResponse
    )
    ;(fetchEventData as jest.Mock).mockResolvedValue(mockEventDataAPIResponse)

    render(
      <MemoryRouter>
        <PopularEventsViewer {...mockProps} />
      </MemoryRouter>
    )
    ...
    expect(fetchEventData).toHaveBeenCalledTimes(
      mockPopularEventIdsApiResponse.length
    )
    expect(fetchEventData).toHaveBeenLastCalledWith(
      mockPopularEventIdsApiResponse[mockPopularEventIdsApiResponse.length - 1]
    )

    expect(mockProps.onUpdatePopularEventsIds).toHaveBeenCalledTimes(1)
    expect(mockProps.onUpdatePopularEventsIds).toHaveBeenCalledWith(
      mockPopularEventIdsApiResponse
    )
    expect(mockProps.onUpdateEventsData).toHaveBeenCalledTimes(1)
    expect(mockProps.onUpdateEventsData).toHaveBeenCalledWith(
      transformEventAPIResponse(mockEventDataAPIResponse)
    )
  })
